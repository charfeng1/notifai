...
**日期:** 2026-01-08
**发现:** 微调模型对训练时的文件夹名称存在强烈偏见，重命名默认文件夹后涌现能力被释放。

### 问题描述

在生产环境测试中发现一个关键问题：**即使添加了完美匹配的自定义文件夹，模型仍然将通知分类到训练时见过的文件夹名称**。

#### 原始行为（问题）

| 测试场景 | 文件夹配置 | 通知示例 | 期望分类 | 实际分类 |
|----------|-----------|----------|----------|----------|
| 添加 Github 文件夹 | Work, Personal, Promotions, Alerts, **Github** | "Your PR was merged" | Github | Work |
| 添加 Appointments 文件夹 | Work, Personal, Promotions, Alerts, **Appointments** | "Doctor visit tomorrow" | Appointments | Personal |
...
模型在微调时"记住"了四个默认文件夹名称（Work, Personal, Promotions, Alerts），形成了**名称偏见**。即使提供了新的文件夹描述，模型仍优先匹配训练时见过的名称模式。

### 实验：重命名默认文件夹

为验证假设，我们将默认文件夹重命名为训练时从未见过的名称：

| 原名称 | 新名称 | 描述（保持不变） |
|--------|--------|-----------------|
| Work | **Job** | Job-related notifications... |
| Personal | **Private** | Family, friends, personal accounts... |
| Promotions | **Deals** | Marketing, sales, deals... |
| Alerts | **Notices** | System alerts, security, deliveries... |

### 结果（重命名后）

| 测试场景 | 文件夹配置 | 通知示例 | 期望分类 | 实际分类 | 状态 |
|----------|-----------|----------|----------|----------|------|
| Github 通知 | Job, Private, Deals, Notices, Github | "Your PR was merged" | Github | **Github** | ✅ |
| 工作邮件 | Job, Private, Deals, Notices | "Meeting at 3pm" | Job | **Job** | ✅ |
| 促销信息 | Job, Private, Deals, Notices | "50% off today" | Deals | **Deals** | ✅ |

**关键发现:** 重命名后，模型开始**阅读并使用文件夹描述**来做分类，而不是依赖记忆中的名称模式。
...
训练时的隐式学习:
├── 模型学会了: "Work" = 工作相关 ← 强关联
├── 模型学会了: "Personal" = 个人相关 ← 强关联
├── 模型学会了: "Promotions" = 促销相关 ← 强关联
└── 模型学会了: "Alerts" = 提醒相关 ← 强关联

推理时的行为:
├── 见到 "Work" → 直接使用训练记忆，忽略描述
├── 见到 "Job" → 无训练记忆，被迫阅读描述
└── 见到 "Github" → 无训练记忆，被迫阅读描述
```

### 为什么这很重要

1. **揭示了微调的副作用** - 模型在获得分类能力的同时，也"记住"了特定的名称
2. **解释了涌现能力的矛盾** - 之前测试显示 73% 新类别准确率，但那是在**完全替换**训练类别的情况下
3. **提供了实用解决方案** - 重命名默认文件夹可以释放模型的涌现能力

### 结论

| 场景 | 模型行为 | 涌现能力 |
|------|----------|----------|
| 使用训练时的文件夹名称 | 依赖名称记忆 | ❌ 被抑制 |
| 完全替换为新名称 | 阅读描述分类 | ✅ 正常工作 |
| 混合（训练名称 + 新名称） | 优先匹配训练名称 | ⚠️ 部分抑制 |

**生产环境建议:**
- ✅ 重命名默认文件夹（避开训练时的名称）
- ✅ 使用清晰、独特的文件夹描述
- ⚠️ 避免使用 "Work", "Personal", "Promotions", "Alerts" 这些精确名称

**状态:** 问题已确认，解决方案已验证（重命名文件夹后分类正常）
